...
def act1_mattie():
    ...
    return (
        f"{char_mati()} stepped into {char_laur()}’s studio just as {char_amr()} stormed past her, muttering angrily in a mix of Ancient Owl and passive-aggressive English, "
        "something about “fraudulent cauldron write-offs” and “pixie-induced hallucinations,” feathers dramatically fluffed. "
        f"The office was left looking like a tornado had hit, with {char_pix()}’s pixie dust absolutely everywhere. {char_laur()} however, was nowhere to be found. "
...
def act2_mattie():
    ...
    from print_amir import char_amr
    return (
        "As Sabrina Woodworker's gentle melodies floated through the air, "
        ...
        f"{char_ali()} reached out and handed him a freshly bloomed forget-me-not, its petals glowing softly as if promising new beginnings. "
...
def act3_mattie():
    ...
    return (
        ...
        f"{char_ali()} and {char_mati()} exchanged delighted smiles, leaning against each other as they watched {char_laur()} finally laugh at one of {char_pix()}’s ridiculous dance moves. "
        f"{char_pix()} twirled around the room, her small feet barely making a sound on the wooden floor, while {char_laur()} tried to imitate her and ended up tumbling onto a pile of cushions. "
